...
/** Lay exported panels out with a white gutter so adjacent ink borders never touch. */
export function stripExportGrid(panelCount: number, panelSize: number, gutter = 6): StripExportGrid {
  ...
  return {
    ...
    position(index) {
      ...
      return {
        x: (index % columns) * (panelSize + gutter),
        y: Math.floor(index / columns) * (panelSize + gutter),
      };
    },
  };
}
